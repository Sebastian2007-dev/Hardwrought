...
public final class CoreSaveData extends SavedData {
    public static final Codec<CoreSaveData> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            // Milestone 11: how much has been drawn out of each oil and gas reservoir. Only the
            // reservoirs anyone has drilled into are here; the rest are as full as they ever were.
            Codec.unboundedMap(Codec.STRING, Codec.LONG.validate(value -> value >= 0
                            ? com.mojang.serialization.DataResult.success(value)
                            : com.mojang.serialization.DataResult.error(() -> "Negative reservoir draw")))
                    .optionalFieldOf("reservoirs", java.util.Map.of())
                    .forGetter(data -> java.util.Map.copyOf(data.reservoirs))
    ).apply(instance, (version, ticks, vitals, cells, aquifers, knowledge, equipment, reservoirs) ->
            new CoreSaveData(ticks, vitals, cells, aquifers, knowledge, equipment, reservoirs)));
    public static final SavedDataType<CoreSaveData> TYPE = new SavedDataType<>(
            ...
            () -> new CoreSaveData(0, java.util.Map.of(), java.util.Map.of(), java.util.Map.of(),
                    java.util.Map.of(), java.util.Map.of(), java.util.Map.of()), CODEC, null);
    ...
    private final java.util.Map<String, Long> reservoirs;
    ...
    private CoreSaveData(long ticks, java.util.Map<String, PlayerVitals> playerVitals,
                         ...
                         java.util.Map<String, Long> reservoirs) {
        this.ticks = ticks;
        ...
        this.reservoirs = new java.util.HashMap<>(reservoirs);
    }

    /** How much has been drawn out of a reservoir so far, in millibuckets or gas units. */
    public long reservoirDrawn(String key) {
        return reservoirs.getOrDefault(key, 0L);
    }

    public void drawFromReservoir(String key, long amount) {
        if (key == null || amount < 0) throw new IllegalArgumentException("Invalid reservoir draw");
        if (amount == 0) return;
        reservoirs.merge(key, amount, Long::sum);
        setDirty();
    }
